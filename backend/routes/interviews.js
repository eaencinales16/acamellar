...
router.post('/', (req, res) => {
  ...
  const result = db.prepare(
    `INSERT INTO interviews (application_id, round, scheduled_at, duration_min, format, interviewers, location, prep_notes, outcome)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(application_id, round || null, scheduled_at || null, duration_min || 60, format || null, interviewers || null, location || null, prep_notes || null, outcome || null);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE interviews SET
    ...
    f.prep_notes ?? existing.prep_notes, f.outcome ?? existing.outcome, req.params.id
  );
...
router.get('/:id/calendar.ics', (req, res) => {
  const iv = db.prepare(withApp + ' WHERE i.id = ?').get(req.params.id);

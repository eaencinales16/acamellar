...
router.get('/', (req, res) => {
  ...
  let query = 'SELECT c.*, a.company as app_company, a.position FROM connections c LEFT JOIN applications a ON c.application_id = a.id';
  const params = [];
  if (application_id) {
    query += ' WHERE c.application_id = ?';
...
router.post('/', (req, res) => {
  ...
  const result = db.prepare(
    'INSERT INTO connections (application_id, name, title, company, linkedin_url, email, notes) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(application_id || null, name, title || null, company || null, linkedin_url || null, email || null, notes || null);
